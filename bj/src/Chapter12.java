...
public class Chapter12 {
    ...
    static void p2839() throws IOException {
        int target = in.nextInt();
        int numberOfFive = target/5;
        while(true) {
            if((target-5*numberOfFive)%3==0) {
                System.out.println(numberOfFive + (target-5*numberOfFive)/3);
                return;
            }
            else numberOfFive--;
            if(numberOfFive<0) break;
        }
