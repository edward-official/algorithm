...
public class Chapter21 {
    ...
    static class P12865 {
        ...
        public static void execute() throws IOException {
            ...
            dp = new int[numberOfItems + 1][capacity + 1];
            ...
            for(int closingItem=1; closingItem<=numberOfItems; closingItem++) {
                for(int closingWeight=1; closingWeight<=capacity; closingWeight++) {
                    int weightOfItem = weights[closingItem];
                    int valueOfItem = values[closingItem];

                    int candidate1 = dp[closingItem - 1][closingWeight];
                    int candidate2 = Integer.MIN_VALUE;
                    if (weightOfItem <= closingWeight) {
                        candidate2 = dp[closingItem - 1][closingWeight - weightOfItem] + valueOfItem;
                    }
                    dp[closingItem][closingWeight] = Math.max(candidate1, candidate2);
                }
            }

            builder.append(dp[numberOfItems][capacity]);
            out.write(builder.toString());
            out.flush();
        }
    }
